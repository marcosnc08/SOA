...
void buzz(unsigned char t, int tone = 50);
...
void loop() {
 ...
 //tiempoActivacion = millis() - tiempoActivacion;
	if(/*millis() - tiempoActivacion > 5000 &&*/ alarmaEncendida == HIGH) {
	  if(millis() - tiempoActivacion > 5000 ) { 
      sensorEstado = digitalRead(sensorPin);
      digitalWrite(ledVerdePin, HIGH);
	  }
    else {
      if(millis() - intermitenciaLed > 500) {
        if(digitalRead(ledVerdePin) == HIGH)
          digitalWrite(ledVerdePin, LOW);
        else
          digitalWrite(ledVerdePin,HIGH);
        intermitenciaLed = millis();
      }
   }
	  //sensorEstado = digitalRead(sensorPin);
	  //digitalWrite(ledVerdePin, HIGH);
